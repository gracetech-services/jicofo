...
const { JingleAction } = require('./jingleConstants');
...
function createRtpHdrExtElements(hdrextList = []) {
    return hdrextList.map(ext => {
        ...
        return createElement('rtp-hdrext', { xmlns: 'urn:xmpp:jingle:apps:rtp:rtp-hdrext:0', ...attrs });
    });
}
...
function createBundleGroupExtension(contentElementsInBundle) {
    ...
    const namedContentElements = contentElementsInBundle.map(contentEl => {
        ...
        return createElement('content', { name: contentEl.attrs.name });
    }).filter(el => el !== null);

    if (namedContentElements.length === 0) return null;

    return createElement('group', { xmlns: 'urn:xmpp:jingle:apps:grouping:0', semantics: 'BUNDLE' }, ...namedContentElements);
}
