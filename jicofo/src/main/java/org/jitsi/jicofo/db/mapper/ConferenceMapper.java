...
public interface ConferenceMapper {
     ...
     * 插入会议信息
     */
    @Insert("INSERT INTO conferences (room_name, meeting_id, created_at, started, include_in_statistics, jvb_version) " +
            "VALUES (#{roomName}, #{meetingId}, #{createdAt}, #{started}, #{includeInStatistics}, #{jvbVersion}) " +
     ...
     * 根据房间名查询会议信息
     */
    @Select("SELECT * FROM conferences WHERE room_name = #{roomName}")
    ConferenceInfo selectByRoomName(@Param("roomName") String roomName);
    
    /**
     * 根据会议ID查询会议信息
     */
    @Select("SELECT * FROM conferences WHERE meeting_id = #{meetingId}")
    ConferenceInfo selectByMeetingId(@Param("meetingId") String meetingId);
    
    /**
     * 更新会议结束时间
     */
    @Update("UPDATE conferences SET ended_at = #{endedAt}, participant_count = #{participantCount} " +
     ...
     * 更新会议状态
     */
    @Update("UPDATE conferences SET started = #{started} WHERE room_name = #{roomName}")
    int updateConferenceStatus(@Param("roomName") String roomName, @Param("started") boolean started);
    
    /**
     * 删除会议
     */
    @Delete("DELETE FROM conferences WHERE room_name = #{roomName}")
    int deleteConference(@Param("roomName") String roomName);
    
    /**
     * 查询所有活跃会议
     */
    @Select("SELECT * FROM conferences WHERE ended_at IS NULL ORDER BY created_at DESC")
    List<ConferenceInfo> selectActiveConferences();
    
    /**
     * 统计会议数量
     */
    @Select("SELECT COUNT(*) FROM conferences WHERE created_at >= #{startTime} AND created_at <= #{endTime}")
